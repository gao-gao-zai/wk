// blackcat.go 夜猫子任务（black_cat）+ 新手礼包/补偿/补签卡 API。
//
// 判据（实测口径）：black_cat 要求在 **23:00–08:00（本地时区）窗口内**完成 3 次
// glm-5.2 对话并上报 chat 事件链；窗口外行为不计分。真实对话走 ChatStream
// （glm-5.2 极短消息），事件链用 ReportChatActivityModel（chat_5 同款上报形状）。
//
