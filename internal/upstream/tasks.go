...
//
// 领奖端点（实测，与列表/接受不同域）：Web 成长中心的领奖请求 ——
//
//	POST https://www.workbuddy.cn/activity/growth/tasks/<task_code>/claim
//	（任务码在**路径**里，无 body；带 x-client-platform: web 头，Bearer 鉴权）
//
// 此前误用 CLI 域 copilot.tencent.com 的 /v2/activity/growth/tasks/reward/claim
// （task_code 放 body），该路径不存在，恒返回 400——这是历史上领奖失败的真实原因。
//
...
// ClaimReward 领取单个任务奖励（Web 域端点，见文件头）。
// 返回 (credit, energy, err)：credit/energy 为本次到账奖励（已领取过时为 0）。
