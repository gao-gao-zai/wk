...
//
// 背景：成长中心连登档位（7d/14d/28d）按连续登录天数解锁，兑换发
...
// 由签到排程（RunCheckinNow）末尾调用；也可控制台手动触发。
...
func (s *Scheduler) streakBonusAccount(a *auth.Auth) {
	...
	if credit, err := cfg.Upstream.ClaimGift(a); err == nil {
		log.Printf("streak-bonus %s: 新手礼包 +%dc", uid, credit)
	}
	if credit, err := cfg.Upstream.ClaimCompensation(a); err == nil {
		log.Printf("streak-bonus %s: 补偿领取 +%dc", uid, credit)
	}

	full, err := cfg.Upstream.GrowthStreakFull(a)
	if err != nil {
		log.Printf("streak-bonus %s: %v", uid, err)
		return
	}
	...
	for _, tier := range full.RedemptionStatus.Tiers {
		...
		if err := cfg.Upstream.GrowthRedeemTier(a, tier.Tier); err != nil {
			// 未解锁（403）属预期，静默；其余记日志。
			log.Printf("streak-bonus %s: redeem %s: %v", uid, tier.Tier, err)
			continue
		}
		log.Printf("streak-bonus %s: ★ 兑换 %s 档（+%dc +%de 卡×%d 抽奖×%d）",
			uid, tier.Tier, tier.Credit, tier.Energy, tier.Cards, tier.Chances)
	}
	// 抽奖：按当前 chances 全抽完（兑换刚发的次数已在服务端累加）。
	chances, err := cfg.Upstream.LotteryChances(a)
	if err != nil {
		log.Printf("streak-bonus %s: lottery summary: %v", uid, err)
		return
	}
	for i := 0; i < chances; i++ {
		raw, err := cfg.Upstream.LotteryDraw(a)
		if err != nil {
			log.Printf("streak-bonus %s: draw: %v", uid, err)
			return
		}
		log.Printf("streak-bonus %s: 第%d抽 %s", uid, i+1, compactJSON(raw))
	}
	if chances > 0 {
		log.Printf("streak-bonus %s: 抽奖完成 %d 次", uid, chances)
	}
}
...
func (s *Scheduler) makeupYesterday(a *auth.Auth) {
	uid := a.Snapshot().UID
	cfg := s.snapshot()
	missed, err := cfg.Upstream.HeatmapYesterdayMissed(a)
	if err != nil || !missed {
		return
	}
	full, err := cfg.Upstream.GrowthStreakFull(a)
	...
	if err := cfg.Upstream.UseMakeupCard(a, yesterday); err != nil {
		log.Printf("streak-bonus %s: 补签 %s 失败: %v", uid, yesterday, err)
		return
	}
	log.Printf("streak-bonus %s: ★ 已用补签卡补签 %s（保连登）", uid, yesterday)
}
